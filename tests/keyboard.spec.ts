...
test("keyboard guide preserves writing focus, resizes and never inserts text", async ({
  ...
  await page.mouse.move(side.x - 14, side.y + 20);
  await page.mouse.up();
  expect((await panel.boundingBox())!.width).toBeCloseTo(initialWidth - 40, 0);
  await expect(editor).toBeFocused();
  ...
  expect(keyboard!.x + keyboard!.width).toBeLessThanOrEqual(bounds!.x);
  await page.getByRole("button", { name: "Режим фокуса", exact: true }).click();
  await expect(panel).toBeVisible();
  await widthResize.focus();
  await widthResize.press("Home");
  await expect(widthResize).toHaveAttribute("aria-valuenow", "300");
  expect(
    await panel
